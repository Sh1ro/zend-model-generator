...
foreach ($tables as $name)
{
	if (!in_array($name, $ignore) and ($limit == '*' or in_array($name, $limit)))
	{
		...
		$info['methods'][] = new Zend_CodeGenerator_Php_Method(array(
				'name' => 'newRow',
				'body' => 'return $this->createRow();',
				'docblock'   => new Zend_CodeGenerator_Php_Docblock(array(
					'shortDescription' => 'Return an empty row',
					'tags'             => array(
						new Zend_CodeGenerator_Php_Docblock_Tag_Return(array(
							'datatype'  => $parser->formatRowClassName($name),
						)),
					),
				))
			)
		...
		$rowClass = new Zend_CodeGenerator_Php_Class(array(
			'name' => $parser->formatRowClassName($name),
			'extendedClass' => $opts->rowclass
		));

		foreach($adapter->describeTable($name) AS $colName => $colData)
		{
			$rowClass->setMethod(array(
				'name'       => "set" . $parser->formatMethodName($colName),
				'parameters' => array(
					array('name' => 'value')
				),
				'body' => '$this->' . $colName . ' = $value;' . "\n" . 'return $this;'
			));

			$rowClass->setMethod(array(
				'name'       => "get" . $parser->formatMethodName($colName),
				'body' => 'return $this->' . $colName . ';'
			));
